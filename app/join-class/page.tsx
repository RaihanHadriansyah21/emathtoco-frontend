...
function JoinClassContent() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      setStatus("idle");
      setMessage(
        "Silakan login sebagai mahasiswa, lalu Anda akan otomatis kembali ke halaman ini.",
      );
      return;
    }
    ...
    const joinClass = async () => {
      ...
      try {
        ...
        setStatus("error");
        setMessage(
          "QR sudah kadaluarsa, dicabut, limit scan habis, atau database belum dimigrasikan.",
        );
      }
    };
  ...
  // Bawa token QR ke halaman login lewat query `redirect`, supaya mahasiswa
  // yang belum punya sesi otomatis kembali ke /join-class?token=... setelah
  // berhasil login (bukan berakhir di dashboard default).
  const loginHref = `/login/mahasiswa?redirect=${encodeURIComponent(`/join-class?token=${token}`)}`;
    ...
    <PageTransition>
      <div className="min-h-screen bg-slate-50 dark:bg-gradient-to-br dark:from-[#060814] dark:via-[#020205] dark:to-[#000000] text-slate-800 dark:text-neutral-200 font-sans flex items-center justify-center px-4 py-10 relative overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-md bg-white/95 border border-slate-200 dark:bg-[#0A0A0F]/90 dark:border-neutral-900 rounded-3xl p-7 shadow-2xl text-center space-y-5">
          ...
          <div>
            <p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-cyan-600 dark:text-cyan-400 mb-2">
              QR Join Kelas
            </p>
            <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">
              Masuk Kelas
            </h1>
            <p className="text-sm text-slate-500 dark:text-neutral-400 mt-2 leading-relaxed">
              {message}
            </p>
          </div>
          ...
          {status === "success" && (
            <button
              type="button"
              onClick={() =>
                router.push(courseId ? `/matkul/${courseId}` : "/")
              }
